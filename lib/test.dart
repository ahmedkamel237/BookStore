...
class Utils {
  ...
  static Conversation flutterChat =
    ...
    Chat(flutter, 'Wow! I can\'t believe this is working on CodePen!',
        1587688792579, true),
  ...
  static Conversation martinChat =
      Conversation(martin, 1587688793050, 1587688793050, [
  ...
  static Conversation nilayChat =
      Conversation(nilay, 1587794105841, 1587794105841, [
    Chat(nilay, 'Hey! What\'s new?', 1587793596393, true),
    Chat(mariano, 'Got something really interesting for #FlutterPen',
        1587793628732, true),
    Chat(mariano, '👀👀👀', 1587793719066, false),
    Chat(mariano, 'You should check on my twitter @geekmz', 1587794105841,
        false),
  ]);

  static Conversation codePenChat =
      Conversation(codePen, 1587842552935, 1587842690825, [
...
class _HomeScreenState extends State<HomeScreen> {
  final GlobalKey<_ChatsScreenState> _chatsMobileKey = GlobalKey();
  final GlobalKey<_ChatsScreenState> _chatsLaptopKey = GlobalKey();
...
class _ChatsScreenState extends State<ChatsScreen> {
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ChatItem(
            ...
            callback: widget.callback ?? (c) {},
            refresh: widget.refresh ?? (r) {},
          );
        },
                ...
                Navigator.of(context)
                  ...
                  if (conversation != null) {
                    if (_userNotInChats(conversation)) {
                      ...
                      jumpToConversation(_conversations
                          .where((element) =>
                              element.user ==
                              (conversation as Conversation).user)
                          .first);
                    }
                  }
                });
  ...
  bool _userNotInChats(Conversation conversation) {
   ...
   _appBar(Size size, bool isMobile) {
      ...
      if (widget.isMobile) {
        setState(() {
          _mobileActions.addAll([
            Utils.action(Icons.search, () {}, widget.isMobile,
                smallDevice: false),
            Utils.action(Icons.more_vert, () {}, widget.isMobile,
                smallDevice: false),
          ]);
        });
      } else {
        setState(() {
          _laptopActions.addAll([
            Utils.action(Icons.play_circle_outline, () {}, widget.isMobile,
                smallDevice: false),
              ...
              Navigator.of(context)
                ...
                if (conversation != null) {
                  if (_userNotInChats(conversation)) {
                    ...
                    jumpToConversation(_conversations
                        .where((element) =>
                            element.user == (conversation as Conversation).user)
                        .first);
                  }
                }
              });
        ...
        child: Row(
          ...
          children: [
            Padding(
              ...
              child:
                  Icon(Icons.search, size: 18, color: Utils.appBarIconLaptop),
            ),
            Expanded(
              child: TextField(
...
class _ChatItemState extends State<ChatItem> {
                          ...
                          child: Row(
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(widget.conversation.chats.isNotEmpty
  ...
  bool _lastMessageMine(Conversation conversation) {
    ...
    return conversation.chats.isNotEmpty
        ? conversation.chats[0].user == Utils.currentUser
        : false;
  }
...
class ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    super.initState();
    _conversation = widget.conversation ?? _conversation;
    Future.delayed(
        Duration(seconds: 0),
        () => setState(
            () => _conversation = widget.conversation ?? _conversation));
    Future.delayed(Duration(seconds: 0), () {
      if (_isMobile) {
        setState(() {
          _mobileActions.addAll([
            Utils.action(Icons.videocam, () {}, _isMobile,
                smallDevice: _smallDevice),
            Utils.action(Icons.call, () {}, _isMobile,
                smallDevice: _smallDevice),
            Utils.action(Icons.more_vert, () {}, _isMobile,
                smallDevice: _smallDevice),
          ]);
        });
      } else {
        setState(() {
          _laptopActions.addAll([
            ...
            Utils.action(Icons.attach_file, () {}, _isMobile,
                smallDevice: false),
                          ...
                          Opacity(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Flexible(
                                  flex: 1,
                                  child: Text('WhatsApp is available for Laptops',
                                      textAlign: TextAlign.center),
                                )
                              ],
                            ),
    ...
    return Align(
      ...
      child: Container(
        margin: EdgeInsets.only(
            left: 20, right: 20, top: chat.firstSection ? 15 : 2),
          ...
          children: [
            Padding(
              padding: EdgeInsets.only(
                  right: _singleEmoji(chat) ? 0 : 12, bottom: 2),
            ...
            Row(
              ...
              children: [
                Opacity(
                  opacity: 0.6,
                  child: Text(_parseTime(chat.timestamp),
                      style: TextStyle(fontSize: 11)),
                ),
                SizedBox(width: 5),
                if (_currentUser(chat))
                  Utils.doubleCheck(_conversation.user.online, true),
              ],
            ),
          ],
        ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          onChanged: (text) {
                            setState(() => _rightAction = text.isNotEmpty
                                ? Icons.send
  ...
  bool _currentUser(Chat chat) => chat.user == Utils.currentUser;
   ...
   _appBar(User user, Size size, bool isMobile) {
    return PreferredSize(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                  height: 2, width: size.width, color: Utils.sectionDivider),
              Padding(
                padding:
              ...
              child: Row(
                children: [
                  ...
                  InkResponse(
                    onTap: () => Navigator.of(context).pop(),
                    child: Icon(Icons.arrow_back,
                        color: Colors.white, size: smallDevice ? 15 : 25),
                  ),
                  SizedBox(width: smallDevice ? 15 : 25),
                  Text('Select contact',
                      style: TextStyle(
...
class ContactScreen extends StatefulWidget {
  ...
  const ContactScreen({super.key, this.user, this.visible = false});
...
class ContactScreenState extends State<ContactScreen> {
  ...
  void initState() {
    ...
    Future.delayed(
        Duration(seconds: 0), () => setState(() => _user = widget.user!));
  }
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Container(
                                ...
                                color: Utils.sectionDivider),
                            contactSection(
                                size, Icons.block, 'Block', Colors.black),
                            contactSection(size, Icons.thumb_down,
                                'Report contact', Colors.red[700]!),
                            contactSection(size, Icons.delete, 'Delete chat',
                                Colors.red[700]!),
                          ],
                        ),
                      ),
          ...
          child: Column(
            children: [
              ...
              Container(
                  height: 2, width: size.width, color: Utils.sectionDivider),
              Container(
                  height: 8, width: size.width, color: Utils.sectionSpacer),
              Container(
                  height: 1, width: size.width, color: Utils.sectionDivider),
            ],
          ),
        ),
      ),
...
Type when<Input, Type>(Input selectedOption, Map<Input, Type> branches,
  ...
  if (!branches.containsKey(selectedOption)) {
    return defaultValue ?? branches.values.first;
  }
  return branches[selectedOption] ?? defaultValue ?? branches.values.first;
}
